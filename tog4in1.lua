...
function window_controls()
    ...
    local first_geo =
        {x = controlbox_left + 20, y = button_y, an = 4, w = buttonWH, h = buttonWH}
    local second_geo =
        {x = controlbox_left + 50, y = button_y, an = 4, w = buttonWH, h = buttonWH}
    local third_geo =
        {x = controlbox_left + 75, y = button_y, an = 4, w = buttonWH, h = buttonWH}
...
function layout()
	...
	if minimalUI then
		...
		lo.geometry = {x = 27, y = refY - oscY - 39 + minimalSeekY, an = 7, w = 50, h = smallIconS}
	end
...
function layoutPot()
    ...
    lo = add_layout("tc_left")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton), y = refY - oscY, an = 4, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton), y = refY - oscY, an = 4, w = 50, h = smallIconS}
	end
    lo.style = osc_styles.timecodeL
	...
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton) + 48, y = refY - oscY, an = 4, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton) + 48, y = refY - oscY, an = 4, w = 50, h = smallIconS}
	end
	lo.style = osc_styles.timecodeR

    lo = add_layout("tc_right")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton) + 56, y = refY - oscY, an = 4, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton) + 56, y = refY - oscY, an = 4, w = 50, h = smallIconS}
	end
    lo.style = osc_styles.timecodeR
	...
	if not minimalUI then
		...
		if user_opts.showIcons then
			...
			-- Playback speed
			lo = add_layout("playback_speed")
			lo.geometry = {x = osc_geo.w - (11 * gapSmallButton) - 5, y = refY - oscY + 0.5, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.smallButtonSpeed
			...
			-- Toggle thumbfast
			lo = add_layout("tog_thumb")
			lo.geometry = {x = osc_geo.w - (8 * gapSmallButton), y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
