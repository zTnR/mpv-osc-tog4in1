...
function layout()
	...
	if not minimalUI then
		...
		if user_opts.showIcons then
			...
			lo.geometry = {x = 110, y = refY - oscY - 1.5, an = 5, w = smallIconS, h = smallIconS}
...
function layoutPot()
	...
	if not minimalUI then
		...
		if user_opts.showIcons then
			...
			-- Playback speed
			lo = add_layout("playback_speed")
			lo.geometry = {x = osc_geo.w - (12 * gapSmallButton) - 5, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.smallButtonSpeed

			-- Toggle tooltip
			lo = add_layout("tog_tooltip")
			lo.geometry = {x = osc_geo.w - (11 * gapSmallButton), y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
