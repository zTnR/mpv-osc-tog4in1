...
local user_opts = {
    ...
    scalewindowed = 2.1,              	-- scaling of the controller when windowed (vidscale false 4K)
    scalefullscreen = 3,            	-- scaling of the controller when fullscreen (vidscale false 4K)
	...
	alphaWinCtrl = 80,					-- opacity windows controls, 0 (opaque) to 255 (transparent)
    seekrangealpha = 64,                -- transparency of seekranges
    seekbarhandlesize = 0,            	-- size ratio of the knob handle
    seekbarkeyframes = true,            -- use keyframes when dragging the seekbar
    hidetimeout = 0,                  	-- duration in ms until the OSC hides ('don't show on mouse move')
    fadeduration = 0,                 	-- duration of fade out in ms, 0 = no fade ('don't show on mouse move')
    hidetimeoutMouseMove = 1000,        -- duration in ms until the OSC hides ('show on mouse move')
    fadedurationMouseMove = 500,        -- duration of fade out in ms, 0 = no fade ('show on mouse move')
...
local darkgrey 			= "464646"	-- Other buttons color (dark grey)
...
local osc_styles = {
	...
	titlePotMini = "{\\blur0\\bord0\\1c&H" .. darkgrey .. "&\\3c&H000000&\\fs16\\fn" .. user_opts.titlefont .. "}",
    ...
    wcButtons = "{\\1c&" .. white .. "&\\fs16\\fnmodernx-osc-icon}",
	wcSwitch = "{\\1c&" .. white .. "&\\fs18\\fnmpv-osd-symbols}",
    wcTitle = "{\\1c&" .. white .. "&\\fs15\\q2\\fn" .. user_opts.titlefont .. "}",
    wcBar = "{\\1c&H" .. black .. "}",
...
local state = {
    ...
    rightTC_trem = user_opts.timetotal, 	-- if the right timecode should display total or remaining time
...
function window_controls()
	...
	-- Switch interface default / minimal
    ne = new_element("switchUI", "button")
    ...
    lo = add_layout("switchUI")
    --lo.geometry = alignment == "left" and fourth_geo or fourth_geo
	lo.geometry = alignment == "left" and third_geo or second_geo
    lo.style = osc_styles.wcSwitch
    ...
    sh_area_y1 = (wc_geo.y + (wc_geo.h / 2)) +
                 get_align(1, osc_param.playresy - (wc_geo.y + (wc_geo.h / 2)), 0, 0)
    add_area("showhide_wc", wc_geo.x, 0, wc_geo.w, sh_area_y1)

    -- Window Title (comms so the whole upper area shows OSC)
...
function layout()
    ...
    -- deadzone above OSC
    local sh_area_y0, sh_area_y1
    sh_area_y0 = get_align(-1, posY - (osc_geo.h/2), 0, 0)
	...
	local bgBarHeight = 1 -- + user_opts.seekbarHeight
	...
	if minimalUI then
		lo.geometry = {x = refX - xMinimalIcons - 95, y = refY - oscY - 39 + minimalSeekY, an = 7, w = 50, h = smallIconS}
		lo.style = osc_styles.timecodeL
	else
		lo.geometry = {x = 25, y = refY - oscY - 39 + minimalSeekY, an = 7, w = 50, h = smallIconS}
		lo.style = osc_styles.timecodeL
	end

    lo = add_layout("tc_right")
	if minimalUI then
		lo.geometry = {x = refX + xMinimalIcons + 50, y = refY - oscY - 39 + minimalSeekY, an = 7, w = 50, h = 200}
	else
		lo.geometry = {x = osc_geo.w - 25, y = refY - oscY - 39 + minimalSeekY, an = 9, w = 50, h = smallIconS}
	end
	...
	lo.style = osc_styles.mediumButtons2
	...
	if not minimalUI then
		...
		-- Title
		geo = {x = 25, y = refY - oscY - 50, an = 1, w = osc_geo.w - 50, h = 48}
		lo = add_layout("title")
		lo.geometry = geo
		...
		lo.geometry = {x = refX - gapNavButton, y = refY - oscY + yMinimalIcons, an = 5, w = smallIconS, h = smallIconS}
		...
		-- if user_opts.showCache then
			-- lo = add_layout("cache")
			-- lo.geometry = {x = 185, y = refY - oscY, an = 5, w = 64, h = smallIconS}
			-- lo.style = osc_styles.smallButtons
		...
		if user_opts.showIcons then
			...
			lo.geometry = {x = osc_geo.w - 135, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
		end
		-- end
...
function layoutPot()
	...
	local smallIconS = user_opts.smallIcon				-- size small icons
	local seekbarHeight = 15 + user_opts.seekbarHeight 	-- full height 27
	local bgBarHeight = 1 -- + user_opts.seekbarHeight
	...
	local seekbarBgAlpha = 150							-- seekbar background transparency
	...
	lo = add_layout("playpause")
	lo.geometry = {x = potRefX, y = refY - oscY - 11, an = 7, w = smallIconS, h = smallIconS}
	lo.style = osc_styles.bigButtonsPot
		
	lo = add_layout("pl_prev")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (3 * gapNavButton), y = refY - oscY - 11, an = 7, w = smallIconS, h = smallIconS}
	else
		lo.geometry = {x = potRefX + gapNavButton, y = refY - oscY - 11, an = 7, w = smallIconS, h = smallIconS}
	end
	lo.style = osc_styles.bigButtonsPot

	lo = add_layout("pl_next")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (4 * gapNavButton), y = refY - oscY - 11, an = 7, w = smallIconS, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (2 * gapNavButton), y = refY - oscY - 11, an = 7, w = smallIconS, h = smallIconS}
	end
	lo.style = osc_styles.bigButtonsPot
    ...
    lo = add_layout("tc_left")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton), y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton), y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	end
    lo.style = osc_styles.timecodeL
	...
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton) + 48, y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton) + 48, y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	end
	lo.style = osc_styles.timecodeR

    lo = add_layout("tc_right")
	if user_opts.showChapters and not minimalUI then
		lo.geometry = {x = potRefX + (5 * gapNavButton) + 56, y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	else
		lo.geometry = {x = potRefX + (3 * gapNavButton) + 56, y = refY - oscY - 8, an = 7, w = 50, h = smallIconS}
	end
    lo.style = osc_styles.timecodeR
	...
	if minimalUI then
		...
		lo.geometry = {x = osc_geo.w - 100, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
	end
	lo.style = osc_styles.smallButtons
	...
	if minimalUI then
		...
		lo.geometry = {x = osc_geo.w - 75, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
	end
	lo.style = osc_styles.smallButtons
	...
	if not minimalUI then
		-- Title
		lo = add_layout("title")
		lo.geometry = {x = potRefX + offsetSeekbarLeft, y = refY - oscY - 8, an = 7, w = seekbarWidth, h = smallIconS}
		...
		if user_opts.showIcons then
			lo = add_layout("playback_speed")
			lo.geometry = {x = osc_geo.w - 280, y = refY - oscY + 1, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.smallButtonSpeed
		end
		...
		if user_opts.showIcons then
			lo = add_layout("tog_tooltip")
			lo.geometry = {x = osc_geo.w - 250, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
		end
		...
		if user_opts.showIcons then
			lo = add_layout("tog_loop")
			lo.geometry = {x = osc_geo.w - 225, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.ctrlLoop
		end
		...
		if user_opts.showIcons then
			lo = add_layout("tog_thumb")
			lo.geometry = {x = osc_geo.w - 200, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
		end
		...
		if user_opts.showIcons then
			lo = add_layout("tog_oscmode")
			lo.geometry = {x = osc_geo.w - 175, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.togTooltip
		end
		...
		if user_opts.showIcons then
			lo = add_layout("tog_ontop")
			lo.geometry = {x = osc_geo.w - 150, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.ctrlLoop
		end
		...
		if user_opts.showIcons then
			lo = add_layout("volume")
			lo.geometry = {x = osc_geo.w - 125, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
			lo.style = osc_styles.volButton
		end

		-- Toggle info
		lo = add_layout("tog_info")
		lo.geometry = {x = osc_geo.w - 50, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
		lo.style = osc_styles.smallButtons

		-- Toggle fullscreen
		lo = add_layout("tog_fs")
		lo.geometry = {x = osc_geo.w - 25, y = refY - oscY, an = 5, w = smallIconS, h = smallIconS}
		lo.style = osc_styles.smallButtons

		-- Cache
		-- if user_opts.showCache then
			-- lo = add_layout("cache")
			-- lo.geometry = {x = osc_geo.w - 350, y = refY - oscY, an = 5, w = 64, h = smallIconS}
			-- lo.style = osc_styles.smallButtons
		-- end
	end
end
...
function osc_init()
	...
	ne.eventresponder["mbtn_right_up"] = function ()
		...
		request_init()
		-- if user_opts.modernTog then
			-- user_opts.showCache = not user_opts.showCache
			-- request_init()
		-- end
	end
...
function process_event(source, what)
    ...
    elseif source == "mouse_move" then
		...
		if (user_opts.minmousemove < 0) then
			if (mouseY >= (osc_param.playresy - user_opts.heightoscShowHidearea)) or (mouseY <= 30)
			then
				show_osc()
			end
